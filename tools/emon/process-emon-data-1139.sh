...
if [ ${edp_architecture_codename} == "cascadelake" ] || [ ${edp_architecture_codename} == "icelake" ] || [ ${edp_architecture_codename} == "sapphirerapids" ]; then
  # CLX, ICX, SPR
  ruby /opt/intel/sep/config/edp/edp.rb -i emon.dat -j emon-v.dat -k emon-M.dat  -m /opt/intel/sep/config/edp/${edp_architecture_codename}_server_${edp_architecture_sockets}_private.xml -f /opt/intel/sep/config/edp/chart_format_${edp_architecture_codename}_server_private.txt -o summary.xlsx --tps $tpstotal -p `nproc` --socket-view

elif [ ${edp_architecture_codename} == "amd" ]; then
  # AMD Milan
  ruby /opt/intel/sep/config/edp/edp.rb -i emon.dat -j emon-v.dat -k emon-M.dat -m /opt/intel/sep/config/edp/${edp_architecture_codename}_${edp_architecture_sockets}_private.xml -f /opt/intel/sep/config/edp/chart_format_${edp_architecture_codename}_private.txt -o summary.xlsx --tps $tpstotal -p `nproc` --socket-view

elif [ ${edp_architecture_codename} == "gracemont" ]; then
  # Alderlake, emon >= 5.41
