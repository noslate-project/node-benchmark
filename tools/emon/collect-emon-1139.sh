...
if [ ${edp_architecture_codename} == "cascadelake" ] || [ ${edp_architecture_codename} == "icelake" ] || [ ${edp_architecture_codename} == "sapphirerapids" ]; then
  # CLX, ICX, SPR
  nohup emon -i /opt/intel/sep/config/edp/${edp_architecture_codename}_server_events_private.txt > emon.dat 2>&1 &

elif [ ${edp_architecture_codename} == "amd" ] || [ ${edp_architecture_codename} == "gracemont" ]; then
  # AMD
  nohup emon -i /opt/intel/sep/config/edp/${edp_architecture_codename}_events_private.txt > emon.dat 2>&1 &
fi
